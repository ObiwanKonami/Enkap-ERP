-- ⚠️  TÜRETILMIŞ DOSYA — migration-runner.ts → CP001_InitialControlPlane.ts ile hizalıdır.
-- Doğrudan düzenleme yerine CP001_InitialControlPlane.ts güncellenmeli, bu dosya ona göre hizalanmalıdır.
-- Docker geliştirme ortamı için convenience init scripti — production'da K8s Job kullanılır.
--
-- Control Plane veritabanı başlangıç şeması (squashed baseline: CP001–CP016)
-- Tenant'lara özel şemalar runtime'da tenant-service tarafından oluşturulur.

-- ─── Roller ───────────────────────────────────────────────────────────────────

CREATE ROLE enkap_provisioner WITH LOGIN PASSWORD 'CHANGE_IN_PRODUCTION';
CREATE ROLE enkap_app         WITH LOGIN PASSWORD 'CHANGE_IN_PRODUCTION';
...
-- CP005 base + CP011 (operational) + CP015 (GİB alias) + CP016 (phone unique index)
...
-- INSERT-ONLY RLS policy (ÖEBSD SIS.5 — loglar değiştirilemez)
...
GRANT SELECT, INSERT, UPDATE ON tenant_routing         TO enkap_provisioner, enkap_app;
GRANT SELECT, INSERT, UPDATE ON tenant_profiles        TO enkap_provisioner, enkap_app;
GRANT SELECT, INSERT, UPDATE ON provisioning_log       TO enkap_provisioner, enkap_app;
GRANT SELECT, INSERT, UPDATE ON subscriptions          TO enkap_app;
GRANT SELECT, INSERT, UPDATE ON billing_plans          TO enkap_app;
GRANT SELECT, INSERT         ON payment_attempts       TO enkap_app;
GRANT SELECT, INSERT, UPDATE ON billing_invoices       TO enkap_app;
GRANT SELECT, INSERT, UPDATE ON webhook_subscriptions  TO enkap_app;
GRANT SELECT, INSERT, UPDATE ON webhook_deliveries     TO enkap_app;
GRANT SELECT, INSERT, UPDATE ON webhook_outbox         TO enkap_app;
GRANT SELECT, INSERT, UPDATE ON white_label_configs    TO enkap_app;
GRANT SELECT, INSERT, UPDATE ON api_clients            TO enkap_app;
GRANT SELECT, INSERT         ON usage_events           TO enkap_app;
GRANT SELECT, INSERT         ON platform_metrics       TO enkap_app;
GRANT SELECT, INSERT, UPDATE ON report_definitions     TO enkap_app;
GRANT SELECT, INSERT, UPDATE ON dashboards             TO enkap_app;
GRANT SELECT, INSERT, UPDATE ON widgets                TO enkap_app;
GRANT SELECT, INSERT         ON gib_audit_logs         TO enkap_app;
GRANT SELECT                 ON fiscal_params           TO enkap_app;
GRANT SELECT, INSERT, UPDATE ON platform_settings      TO enkap_provisioner;
GRANT SELECT, INSERT, UPDATE ON platform_admins        TO enkap_provisioner;
