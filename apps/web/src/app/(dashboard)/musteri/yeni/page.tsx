...
import { crmApi, type ContactType } from '@/services/crm';
...
export default function YeniMusteriPage() {
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: (res) => {
      router.push(`/musteri/${res.data.id}`);
    },
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center gap-3">
        <Button variant="outline" size="sm" className="gap-1.5 h-9 text-xs" asChild>
          <Link href="/musteri">
            <ArrowLeft size={13}/> {t('crm.contacts')}
          </Link>
        </Button>
        <h1 className="text-xl font-bold tracking-tight text-foreground flex items-center gap-2">
          <Users size={20} className="text-muted-foreground"/> {t('crm.newContact')}
        </h1>
      </div>
      ...
      <div className="flex flex-col gap-4 max-w-[780px]">
        ...
        <Card className="p-5">
          <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-4 flex items-center gap-1.5">
            {kind === 'B2B' ? <Building2 size={12}/> : <User size={12}/>}
